...
__published:	// IDE-managed Components
	...
	TVirtualImage *VirtualImage3;           // StatusComboBoxChange
	...
	void __fastcall LinkControlToField9AssignedValue( TObject* Sender, TBindingAssignValueRec &AssignValueRec,
		  const TValue &Value );
	void __fastcall StatusComboBoxChange(TObject *Sender);
private:	// User declarations
public:		// User declarations
	 void UpdateFieldsVisibility(); // for viewing or not leads
    ...
    bool LocateLead1(int leadId) {
        return LeadsBindSourceDB->DataSet->Locate("LeadId", leadId, TLocateOptions());
	}
