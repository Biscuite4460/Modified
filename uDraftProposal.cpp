...
void __fastcall TDraftProposalForm::RefreshFormsPublic(){
	Invalidate();
}
...
{
    if (Field != NULL && (Field->FieldName == L"Quantity" || Field->FieldName == L"Price"))
    {
		Application->MessageBox(L"Relevant data change detected", L"Notification", MB_OK);
...
void __fastcall TDraftProposalForm::PopulateLineItemsFromGrid()
{
	DM->LineItemsFDMemTable->EmptyDataSet();

	for (int row = 1; row < LineItemsStringGrid->RowCount; ++row)
	{
		int quantity = StrToIntDef(LineItemsStringGrid->Cells[0][row], 0);
		double price = StrToFloatDef(LineItemsStringGrid->Cells[1][row], 0.0);

		DM->LineItemsFDMemTable->Append();
		DM->LineItemsFDMemTable->FieldByName(L"Quantity")->AsInteger = quantity;
		DM->LineItemsFDMemTable->FieldByName(L"Price")->AsFloat = price;
		DM->LineItemsFDMemTable->Post();
	}
}

void __fastcall TDraftProposalForm::UpdateTotalDirectly()
{
	double total = 0.0;

    if (!DM->LineItemsFDMemTable->IsEmpty())
    {
		DM->LineItemsFDMemTable->DisableControls();
        DM->LineItemsFDMemTable->First();
        while (!DM->LineItemsFDMemTable->Eof)
		{
            int quantity = DM->LineItemsFDMemTable->FieldByName("Quantity")->AsInteger;
			double price = DM->LineItemsFDMemTable->FieldByName("Price")->AsFloat;
            total += static_cast<double>(quantity) * price;

			DM->LineItemsFDMemTable->Next();
        }

		DM->LineItemsFDMemTable->EnableControls();
    }

    TotalEdit->Text = FormatFloat("0.00", total);
